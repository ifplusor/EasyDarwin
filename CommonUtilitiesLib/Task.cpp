...
Task::Task()
        : fEvents(0), fUseThisThread(nullptr), fDefaultThread(nullptr), fWriteLock(false), fTimerHeapElem(),
          fTaskQueueElem(), pickerToUse(&Task::sShortTaskThreadPicker)
{
#if DEBUG
    fInRunCount = 0;
#endif
...
void Task::Signal(EventFlags events)
{
    ...
    if ((!(oldEvents & kAlive)) && (TaskThreadPool::sNumTaskThreads > 0)) {
        ...
        if (fUseThisThread != nullptr)// Task needs to be placed on a particular thread.
        {
            ...
            if (TASK_DEBUG) {
                ...
                qtss_printf("Task::Signal EnQueue TaskName=%s fUseThisThread=%p q_elem=%p enclosing=%p\n", fTaskName,
                            (void *) fUseThisThread, (void *) &fTaskQueueElem, (void *) this);
            ...
            if (&Task::sShortTaskThreadPicker == pickerToUse) {
                ...
                if (TASK_DEBUG)
                    qtss_printf(
                            ...
                            fTaskName, Task::sShortTaskThreadPicker, TaskThreadPool::sNumShortTaskThreads,
                            TaskThreadPool::sNumShortTaskThreads - 1, theThreadIndex);
                ...
                if (TASK_DEBUG)
                    qtss_printf(
                            ...
                            fTaskName, Task::sBlockingTaskThreadPicker, TaskThreadPool::sNumBlockingTaskThreads,
                            TaskThreadPool::sNumShortTaskThreads,
                            TaskThreadPool::sNumBlockingTaskThreads + TaskThreadPool::sNumShortTaskThreads - 1,
                            theThreadIndex);
            } else {
        ...
        qtss_printf("Task::Signal Sent to dead TaskName=%s  q_elem=%p  enclosing=%p\n", fTaskName,
                    (void *) &fTaskQueueElem, (void *) this);
}
            ...
            if (theTimeout < 0) {
                ...
                if (TASK_DEBUG)
                    qtss_printf(
                            ...
                            theTask->fTaskName, (void *) this, (void *) &theTask->fTimerHeapElem, (void *) theTask,
        ...
        if ((fHeap.PeekMin() != nullptr) && (fHeap.PeekMin()->GetValue() <= theCurrentTime)) {
            if (TASK_DEBUG)
                qtss_printf(
                        ...
                        ((Task *) fHeap.PeekMin()->GetEnclosingObject())->fTaskName, (void *) this,
                        fHeap.CurrentHeapSize(), (void *) fHeap.PeekMin(),
        ...
        if (theElem != nullptr) {
            if (TASK_DEBUG)
                qtss_printf(
                        ...
                        ((Task *) theElem->GetEnclosingObject())->fTaskName, (void *) this,
                        fTaskQueue.GetQueue()->GetLength(), (void *) theElem, (void *) theElem->GetEnclosingObject());
