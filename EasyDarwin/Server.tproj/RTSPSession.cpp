...
RTSPSession::~RTSPSession()
{
    ...
    for (UInt32 x = 0; x < QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPSessionClosingRole); x++)
        (void) QTSServerInterface::GetModule(QTSSModule::kRTSPSessionClosingRole, x)->CallDispatch(
    ...
    while (this->IsLiveSession()) {
        ...
        switch (fState) {
            ...
            case kHTTPFilteringRequest: {
                ...
                if (preFilterErr == QTSS_NoErr) { HTTP_TRACE("RTSPSession::Run kHTTPFilteringRequest\n")
                    continue;
                } else {
            ...
            case kFilteringRequest: {
                ...
                for (; (fCurrentModule < numModules) &&
                       ((!fRequest->HasResponseBeenSent()) || fModuleState.eventRequested); fCurrentModule++) {
            ...
            case kAuthenticatingRequest: {
                ...
                if (lastUsedDigestChallengePtr != nullptr)
                    (void) fRequest->SetValue(qtssRTSPReqDigestChallenge, (UInt32) 0,
                                              (void *) lastUsedDigestChallengePtr->Ptr, lastUsedDigestChallengePtr->Len,
                                              QTSSDictionary::kDontObeyReadOnly);

                numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPAthnRole);
                for (fCurrentModule = 0; (fCurrentModule < numModules) && ((!fRequest->HasResponseBeenSent()) ||
                ...
                if (fRTPSession != nullptr) {
                    ...
                    numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPPostProcessorRole);
                    {
                        ...
                        for (; (fCurrentModule < numModules) || (fModuleState.eventRequested); fCurrentModule++) {
                            ...
                            theModule = QTSServerInterface::GetModule(QTSSModule::kRTSPPostProcessorRole,
                                                                      fCurrentModule);
...
bool RTSPSession::ParseProxyTunnelHTTP()
{
    ...
    */
    ...
    if (splRequest) {
        ...
        if (fHTTPMethod != kHTTPMethodUnknown) { HTTP_VTRACE("IsAHTTPProxyTunnelPostRequest found POST or GET\n")
            ...
            // 识别是否有http字段
            if (theParsedData.NumEqualIgnoreCase("http", 4)) { HTTP_TRACE("ParseProxyTunnelHTTP found HTTP\n")
                ...
                if (theParsedData.EqualIgnoreCase(kSessionHeaderName, kSessionHeaderNameLen)) {
                    ...
                    if (parser.GetDataRemaining() > 0) {
                        ...
                        if (sessionID.Len < QTSS_MAX_SESSION_ID_LENGTH) {
                            ...
                            fProxySessionIDPtr.Set(fProxySessionID, ::strlen(fProxySessionID));HTTP_VTRACE_ONE(
                                    "found session id: %s\n", fProxySessionID)
                        }
                    }
                }
...
QTSS_Error RTSPSession::PreFilterForHTTPProxyTunnel()
{
    ...
    if (rtspSessionRef == nullptr) { HTTP_TRACE("RegisterRTSPSessionIntoHTTPProxyTunnelMap returned nullptr. Abort.\n");
        return QTSS_RequestFailed;
    }

    // We registered ourselves into the map (we are the first half), so wait for our other half
    if (rtspSessionRef == &fProxyRef) { HTTP_TRACE("Registered this session into map. Waiting to bind\n");
        return QTSS_NoErr;
    }
...
void RTSPSession::CheckAuthentication()
{
    ...
    if (scheme != (fRTPSession->GetAuthScheme())) {
        ...
        do {
            ...
            if (qop == RTSPSessionInterface::kAuthQop) {
                ...
                CalcRequestDigest(userPassword, nonce, nonceCount, cNonce, &sAuthQop, &method, digestUri, &emptyStr,
                                  &requestDigest);
                ...
                // allocates memory for requestDigest->Ptr
                CalcRequestDigest(userPassword, nonce, &emptyStr, &emptyStr, &emptyStr, &method, digestUri, &emptyStr,
                                  &requestDigest);
            }
    ...
    if (!fRequest->GetAuthHandled()) {
        if ((!authenticated) || (authenticated && (fRequest->GetStale()))) {
            ...
            (void) profile->SetValue(qtssUserPassword, 0, sEmptyStr.Ptr, sEmptyStr.Len,
                                     QTSSDictionary::kDontObeyReadOnly);
...
void RTSPSession::SetupRequest()
{
    ...
    UInt32 headerBits = fRequest->GetBandwidthHeaderBits();
    if (headerBits != 0)
        (void) fRTPSession->SetValue(qtssCliSessLastRTSPBandwidth, 0, &headerBits, sizeof(headerBits),
                                     QTSSDictionary::kDontObeyReadOnly);
...
void RTSPSession::SetupClientSessionAttrs()
{
    ...
    (void) fRTPSession->SetValue(qtssCliSesPresentationURL, 0, theValue->Ptr, theValue->Len,
                                 QTSSDictionary::kDontObeyReadOnly);
    ...
    Assert(theValue != nullptr);
    (void) fRTPSession->SetValue(qtssCliSesHostName, 0, theValue->Ptr, theValue->Len,
                                 QTSSDictionary::kDontObeyReadOnly);

    // get and pass user agent header
    theValue = fRequest->GetHeaderDictionary()->GetValue(qtssUserAgentHeader);
    Assert(theValue != nullptr);
    (void) fRTPSession->SetValue(qtssCliSesFirstUserAgent, 0, theValue->Ptr, theValue->Len,
                                 QTSSDictionary::kDontObeyReadOnly);
    ...
    if (fRequest->GetMethod() == qtssDescribeMethod) {
        ...
        Assert(theValue != nullptr);
        (void) fRTPSession->SetValue(qtssCliSesReqQueryString, 0, theValue->Ptr, theValue->Len,
                                     QTSSDictionary::kDontObeyReadOnly);
    }
    ...
    (void) fRTPSession->SetValue(qtssCliRTSPSessRemoteAddrStr, (UInt32) 0, tempStr.Ptr, tempStr.Len,
                                 QTSSDictionary::kDontObeyReadOnly);

    tempStr.Len = 0;
    (void) this->GetValuePtr(qtssRTSPSesLocalDNS, (UInt32) 0, (void **) &tempStr.Ptr, &tempStr.Len);
    Assert(tempStr.Len != 0);
    (void) fRTPSession->SetValue(qtssCliRTSPSessLocalDNS, (UInt32) 0, (void **) tempStr.Ptr, tempStr.Len,
                                 QTSSDictionary::kDontObeyReadOnly);

    tempStr.Len = 0;
    (void) this->GetValuePtr(qtssRTSPSesLocalAddrStr, (UInt32) 0, (void **) &tempStr.Ptr, &tempStr.Len);
    Assert(tempStr.Len != 0);
    (void) fRTPSession->SetValue(qtssCliRTSPSessLocalAddrStr, (UInt32) 0, tempStr.Ptr, tempStr.Len,
                                 QTSSDictionary::kDontObeyReadOnly);
}
...
UInt32 RTSPSession::GenerateNewSessionID(char *ioBuffer)
{
...

    {
        ...
        if (theHashTable->GetNumEntries() > 0) {
            ...
            for (UInt32 theCount = 0; theCount < theFirstRandom; theIter.Next(), theCount++) Assert(!theIter.IsDone());
...
void RTSPSession::SaveRequestAuthorizationParams(RTSPRequest *theRTSPRequest)
{
    ...
    if (tempPtr) {
        (void) this->SetValue(qtssRTSPSesLastUserName, 0, tempPtr->Ptr, tempPtr->Len,
                              QTSSDictionary::kDontObeyReadOnly);
        (void) fRTPSession->SetValue(qtssCliRTSPSesUserName, (UInt32) 0, tempPtr->Ptr, tempPtr->Len,
                                     QTSSDictionary::kDontObeyReadOnly);
    }
    ...
    Assert(tempPtr != nullptr);
    if (tempPtr) {
        (void) this->SetValue(qtssRTSPSesLastUserPassword, 0, tempPtr->Ptr, tempPtr->Len,
                              QTSSDictionary::kDontObeyReadOnly);
        (void) fRTPSession->SetValue(qtssCliRTSPSesUserPassword, (UInt32) 0, tempPtr->Ptr, tempPtr->Len,
                                     QTSSDictionary::kDontObeyReadOnly);
    }
    ...
    if (tempPtr) {
        if (tempPtr->Len == 0) {
            ...
            (void) fRTPSession->SetValue(qtssCliRTSPSesURLRealm, (UInt32) 0, realm, len,
                                         QTSSDictionary::kDontObeyReadOnly);
        } else {
            (void) this->SetValue(qtssRTSPSesLastURLRealm, 0, tempPtr->Ptr, tempPtr->Len,
                                  QTSSDictionary::kDontObeyReadOnly);
            (void) fRTPSession->SetValue(qtssCliRTSPSesURLRealm, (UInt32) 0, tempPtr->Ptr, tempPtr->Len,
                                         QTSSDictionary::kDontObeyReadOnly);
        }
    }
}
