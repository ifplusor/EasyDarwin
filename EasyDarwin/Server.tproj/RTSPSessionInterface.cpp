...
void RTSPSessionInterface::Initialize()
{
    for (UInt32 x = 0; x < qtssRTSPSesNumParams; x++)
        QTSSDictionaryMap::GetMap(QTSSDictionaryMap::kRTSPSessionDictIndex)->
                SetAttribute(x, sAttributes[x].fAttrName, sAttributes[x].fFuncPtr, sAttributes[x].fAttrDataType,
...
RTSPSessionInterface::InterleavedWrite(void *inBuffer, UInt32 inLen, UInt32 *outLenWritten, unsigned char channel)
{
    ...
    if ((inLen > kTCPCoalesceDirectWriteSize || inLen == 0) && fNumInCoalesceBuffer > 0
        ...
        err = this->GetOutputStream()->WriteV(iov, 2, fNumInCoalesceBuffer, &buffLenWritten,
    ...
    if (err == QTSS_NoErr) {
        ...
        if (inLen > kTCPCoalesceDirectWriteSize) {
            ...
            err = this->GetOutputStream()->WriteV(iov, 3, inLen + sizeof(rih), outLenWritten,
                                                  RTSPResponseStream::kAllOrNothing);
...
void *RTSPSessionInterface::SetupParams(QTSSDictionary *inSession, UInt32 * /*outLen*/)
{
    ...
    if (theLocalAddrStr == nullptr || theLocalDNSStr == nullptr || theRemoteAddrStr ==
