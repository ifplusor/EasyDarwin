...
void LogStatus(QTSS_ServerState theServerState)
{
    ...
    if (statusFile != NULL) {
        ...
        for (i = 0; i < numAttributes; i++) {
            (void) QTSS_GetValueAsString(sServer, QTSSModuleUtils::GetAttrID(sServer, sAttributes[i]), 0,
                                         &theAttributeValue);
...
void RunServer()
{
    ...
    while ((theServerState != qtssShuttingDownState) &&
        ...
        if ((sServer->SigIntSet()) || (sServer->SigTermSet())) {
            ...
            (void) QTSS_SetValue(QTSServerInterface::GetServer(), qtssSvrState, 0, &theServerState,
    ...
    for (UInt32 shutdownWaitCount = 0;
         (sServer->GetNumRTPSessions() > 0) && (shutdownWaitCount < 5); shutdownWaitCount++)
        OSThread::Sleep(1000);
