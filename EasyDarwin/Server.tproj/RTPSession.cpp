...
RTPSession::~RTPSession()
{
    ...
    if (QTSServerInterface::GetServer()->GetPrefs()->GetReliableUDPPrintfsEnabled()) {
        ...
        for (int x = 0;
             this->GetValuePtr(qtssCliSesStreamObjects, x, (void **) &theStream, &theLen) == QTSS_NoErr; x++) {
        ...
        qtss_printf(
                ...
                tracker->GetMaxCongestionWindowSize(), tracker->GetMinCongestionWindowSize(),
...
QTSS_Error RTPSession::Activate(const StrPtrLen &inSessionID)
{
    ...
    err = theServer->SetValue(qtssSvrClientSessions, theServer->GetNumRTPSessions(), &theSession, sizeof(theSession),
    ...
    if ((events & Task::kKillEvent) || (events & Task::kTimeoutEvent) || (fModuleDoingAsyncStuff)) {
        ...
        UInt32 numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kClientSessionClosingRole);
        {
            for (; fCurrentModule < numModules; fCurrentModule++) {
                ...
                QTSSModule *theModule = QTSServerInterface::GetModule(QTSSModule::kClientSessionClosingRole,
                                                                      fCurrentModule);
    ...
    // RTSP requests coming in while it's sending packets
    {
        ...
        if (fNextSendPacketsTime > theParams.rtpSendPacketsParams.inCurrentTime) {
            ...
            for (int streamIter = 0;
                 this->GetValuePtr(qtssCliSesStreamObjects, streamIter, (void **) &retransStream, &retransStreamLen) ==
            ...
            fNextSendPacketsTime =
                    theParams.rtpSendPacketsParams.inCurrentTime + theParams.rtpSendPacketsParams.outNextPacketTime;
        }
