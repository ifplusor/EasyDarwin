...
QTSS_Error QTSSDictionary::SetValue(QTSS_AttributeID inAttrID, UInt32 inIndex,
                                    const void *inBuffer, UInt32 inLen, UInt32 inFlags)
{
    // Check first to see if this is a static attribute or an instance attribute
    QTSSDictionaryMap *theMap = fMap;
    DictValueElement *theAttrs = fAttributes;
...
QTSS_Error QTSSDictionary::RemoveValue(QTSS_AttributeID inAttrID, UInt32 inIndex, UInt32 inFlags)
{
    ...
    if (theAttrs[theMapIndex].fIsDynamicDictionary) {
        ...
        QTSSDictionary *dict = *(QTSSDictionary **) (theAttrs[theMapIndex].fAttributeData.Ptr +
                                                     (theValueLen * inIndex));
        delete dict;
    }
...
QTSS_Error QTSSDictionary::GetAttrInfoByID(QTSS_AttributeID inAttrID, QTSSAttrInfoDict **outAttrInfoDict)
{
    ...
    } else if (fMap != NULL) return fMap->GetAttrInfoByID(inAttrID, outAttrInfoDict);
...
void QTSSDictionaryMap::Initialize()
{
    ...
    sDictionaryMaps[kModulePrefsDictIndex] = new QTSSDictionaryMap(0, QTSSDictionaryMap::kInstanceAttrsAllowed |
                                                                      QTSSDictionaryMap::kCompleteFunctionsAllowed);
...
QTSSDictionaryMap::QTSSDictionaryMap(UInt32 inNumReservedAttrs, UInt32 inFlags)
        : fNextAvailableID(inNumReservedAttrs), fNumValidAttrs(inNumReservedAttrs), fAttrArraySize(inNumReservedAttrs),
          fFlags(inFlags)
{
                                     ...
                                     QTSS_AttrPermission inPermission)
{
    ...
    fAttrArray[theIndex]->SetVal(qtssAttrDataType, &fAttrArray[theIndex]->fAttrInfo.fAttrDataType,
...
UInt32 QTSSDictionaryMap::GetMapIndex(QTSS_ObjectType inType)
{
    ...
    switch (inType) {
        case qtssRTPStreamObjectType:
            return kRTPStreamDictIndex;
        case qtssClientSessionObjectType:
            return kClientSessionDictIndex;
        case qtssRTSPSessionObjectType:
            return kRTSPSessionDictIndex;
        case qtssRTSPRequestObjectType:
            return kRTSPRequestDictIndex;
        case qtssRTSPHeaderObjectType:
            return kRTSPHeaderDictIndex;
        case qtssServerObjectType:
            return kServerDictIndex;
        case qtssPrefsObjectType:
            return kPrefsDictIndex;
        case qtssTextMessagesObjectType:
            return kTextMessagesDictIndex;
        case qtssFileObjectType:
            return kFileDictIndex;
        case qtssModuleObjectType:
            return kModuleDictIndex;
        case qtssModulePrefsObjectType:
            return kModulePrefsDictIndex;
        case qtssAttrInfoObjectType:
            return kAttrInfoDictIndex;
        case qtssUserProfileObjectType:
            return kQTSSUserProfileDictIndex;
        case qtssConnectedUserObjectType:
            return kQTSSConnectedUserDictIndex;

        case easyHTTPSessionObjectType:
            return kHTTPSessionDictIndex;
