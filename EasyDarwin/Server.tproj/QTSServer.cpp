...
bool QTSServer::CreateListeners(bool startListeningNow, QTSServerPrefs *inPrefs, UInt16 inPortOverride)
{
    ...
    for (UInt32 count3 = 0; count3 < theTotalRTSPPortTrackers; count3++) {
        if (theRTSPPortTrackers[count3].fNeedsCreating) {
            ...
            QTSS_Error err = newListenerArray[curPortIndex]->Initialize(theRTSPPortTrackers[count3].fIPAddr,
    ...
    for (UInt32 count3 = 0; count3 < theTotalHTTPPortTrackers; count3++) {
        if (theHTTPPortTrackers[count3].fNeedsCreating) {
            ...
            QTSS_Error err = newListenerArray[curPortIndex]->Initialize(theHTTPPortTrackers[count3].fIPAddr,
    ...
    for (UInt32 count6 = 0; count6 < fNumListeners; count6++) {
        if (fListeners[count6]->GetLocalAddr() != INADDR_LOOPBACK) {
            ...
            (void) this->SetValue(qtssSvrRTSPPorts, portIndex, &thePort, sizeof(thePort),
    ...
    if (::strlen(runGroupName.GetObject()) > 0) {
        ...
        if (gr == nullptr || ::setgid(gr->gr_gid) == -1) {
            ...
            QTSSModuleUtils::LogError(qtssFatalVerbosity, qtssMsgCannotSetRunGroup, 0,
                                      ...
                                      qtss_strerror(OSThread::GetErrno(), buffer, sizeof(buffer)));
            return false;
        }
        groupID = gr->gr_gid;
    }
...
bool QTSServer::AddModule(QTSSModule *inModule)
{
    ...
    QTSS_Error theErr = this->SetValue(qtssSvrModuleObjects, theNumModules, &inModule, sizeof(QTSSModule *),
                                       QTSSDictionary::kDontObeyReadOnly);
    Assert(theErr == QTSS_NoErr);
...
QTSS_Error QTSServer::RereadPrefsService(QTSS_ServiceFunctionArgsPtr /*inArgs*/)
{
    ...
    for (int y = 0;
         QTSServerInterface::GetServer()->GetValuePtr(qtssSvrModuleObjects, y, (void **) &theModule, &theLen) ==
         QTSS_NoErr; y++) {
