...
bool RTPSessionOutput::FilterPacket(QTSS_RTPStreamObject *theStreamPtr, StrPtrLen *inPacket)
{
    ...
    QTSS_Error writeErr = QTSS_GetValuePtr(*theStreamPtr, sStreamPacketCountAttr, 0, (void **) &packetCountPtr,
                                           &theLen);
...
bool RTPSessionOutput::PacketAlreadySent(QTSS_RTPStreamObject *theStreamPtr, UInt32 inFlags, UInt64 *packetIDPtr)
{
    ...
    if (inFlags & qtssWriteFlagsIsRTP) {
        if ((QTSS_NoErr ==
             QTSS_GetValuePtr(*theStreamPtr, sLastRTPPacketIDAttr, 0, (void **) &lastPacketIDPtr, &theLen))
...
QTSS_Error RTPSessionOutput::TrackRTCPBaseTime(QTSS_RTPStreamObject *theStreamPtr, StrPtrLen *inPacketStrPtr,
                                               SInt64 *currentTimePtr, UInt32 inFlags, SInt64 *packetLatenessInMSec,
                                               SInt64 *timeToSendThisPacketAgain, UInt64 *packetIDPtr,
                                               SInt64 *arrivalTimeMSecPtr)
{
    ...
    if (!fMustSynch || QTSS_NoErr == QTSS_GetValue(*theStreamPtr, sBaseRTPTimeStampAttr, 0, (void *) &baseTimeStamp,
                                                   &theLen)) // we need a starting stream time that is synched
    {
        ...
        if (fMustSynch || QTSS_NoErr !=
                          QTSS_GetValuePtr(*theStreamPtr, sBaseArrivalTimeStampAttr, 0, (void **) &fBaseArrivalTime,
                                           &theLen)) {   // we don't have a base arrival time for the session see if we can set one now.

            for (SInt32 z = 0;
                 QTSS_GetValuePtr(fClientSession, qtssCliSesStreamObjects, z, (void **) &findStream, &theLen) ==
                 QTSS_NoErr; z++) {
                SInt64 *firstArrivalTimePtr = NULL;
                if (QTSS_NoErr !=
                    QTSS_GetValuePtr(*findStream, sFirstRTPArrivalTimeAttr, 0, (void **) &firstArrivalTimePtr,
            ...
            if (haveAllFirstRTPs) // we can now create a base arrival time and base stream time from that
            {
                ...
                writeErr = QTSS_SetValue(*theStreamPtr, sBaseArrivalTimeStampAttr, 0, &earliestArrivalTime,
                                         sizeof(SInt64));
                Assert(writeErr == QTSS_NoErr);
        ...
        {   // we don't have a base stream time but we have a base session time so calculate the base stream time.
            ...
            if (QTSS_NoErr !=
                QTSS_GetValue(*theStreamPtr, sFirstRTPTimeStampAttr, 0, (void *) &firstStreamTime, &theLen))
                return QTSS_NoErr;

            theLen = sizeof(firstStreamArrivalTime);
            if (QTSS_NoErr !=
                QTSS_GetValue(*theStreamPtr, sFirstRTPArrivalTimeAttr, 0, (void *) &firstStreamArrivalTime, &theLen))
                return QTSS_NoErr;
            ...
            UInt32 timeDiffStreamTime = (UInt32) (((Float64) arrivalTimeDiffMSecs / (Float64) 1000.0) *
                                                  (Float64) streamTimeScale);
            baseTimeStamp = firstStreamTime - timeDiffStreamTime;
            if (QTSS_NoErr ==
                QTSS_SetValue(*theStreamPtr, sBaseRTPTimeStampAttr, 0, (void *) &baseTimeStamp, sizeof(baseTimeStamp)))
                haveBaseTime = true;
...
QTSS_Error
RTPSessionOutput::RewriteRTCP(QTSS_RTPStreamObject *theStreamPtr, StrPtrLen *inPacketStrPtr, SInt64 *currentTimePtr,
                              UInt32 inFlags, SInt64 *packetLatenessInMSec, SInt64 *timeToSendThisPacketAgain,
                              UInt64 *packetIDPtr, SInt64 *arrivalTimeMSecPtr)
{
    UInt32 theLen;
    ...
    *theNTPTimestampP = OS::HostToNetworkSInt64(OS::TimeMilli_To_1900Fixed64Secs(*currentTimePtr)); // time now
    ...
    UInt32 baseTimeStamp = 0;
    theLen = sizeof(baseTimeStamp);
    (void) QTSS_GetValue(*theStreamPtr, sBaseRTPTimeStampAttr, 0, (void *) &baseTimeStamp,
                         &theLen); // we need a starting stream time that is synched
...
QTSS_Error RTPSessionOutput::TrackRTCPPackets(QTSS_RTPStreamObject *theStreamPtr, StrPtrLen *inPacketStrPtr,
                                              SInt64 *currentTimePtr, UInt32 inFlags, SInt64 *packetLatenessInMSec,
                                              SInt64 *timeToSendThisPacketAgain, UInt64 *packetIDPtr,
                                              SInt64 *arrivalTimeMSecPtr)
{
    ...
    this->TrackRTCPBaseTime(theStreamPtr, inPacketStrPtr, currentTimePtr, inFlags, packetLatenessInMSec,
                            timeToSendThisPacketAgain, packetIDPtr, arrivalTimeMSecPtr);

    this->RewriteRTCP(theStreamPtr, inPacketStrPtr, currentTimePtr, inFlags, packetLatenessInMSec,
                      timeToSendThisPacketAgain, packetIDPtr, arrivalTimeMSecPtr);
...
QTSS_Error
RTPSessionOutput::TrackRTPPackets(QTSS_RTPStreamObject *theStreamPtr, StrPtrLen *inPacketStrPtr, SInt64 *currentTimePtr,
                                  UInt32 inFlags, SInt64 *packetLatenessInMSec, SInt64 *timeToSendThisPacketAgain,
                                  UInt64 *packetIDPtr, SInt64 *arrivalTimeMSecPtr)
{
    QTSS_Error writeErr = QTSS_NoErr;
...
QTSS_Error
RTPSessionOutput::TrackPackets(QTSS_RTPStreamObject *theStreamPtr, StrPtrLen *inPacketStrPtr, SInt64 *currentTimePtr,
                               UInt32 inFlags, SInt64 *packetLatenessInMSec, SInt64 *timeToSendThisPacketAgain,
                               UInt64 *packetIDPtr, SInt64 *arrivalTimeMSecPtr)
{
    if (this->IsUDP())
    ...
    if (inFlags & qtssWriteFlagsIsRTCP)
        (void) this->TrackRTCPPackets(theStreamPtr, inPacketStrPtr, currentTimePtr, inFlags, packetLatenessInMSec,
                                      timeToSendThisPacketAgain, packetIDPtr, arrivalTimeMSecPtr);
    else if (inFlags & qtssWriteFlagsIsRTP)
        (void) this->TrackRTPPackets(theStreamPtr, inPacketStrPtr, currentTimePtr, inFlags, packetLatenessInMSec,
                                     timeToSendThisPacketAgain, packetIDPtr, arrivalTimeMSecPtr);
...
QTSS_Error
RTPSessionOutput::WritePacket(StrPtrLen *inPacket, void *inStreamCookie, UInt32 inFlags, SInt64 packetLatenessInMSec,
                              SInt64 *timeToSendThisPacketAgain, UInt64 *packetIDPtr, SInt64 *arrivalTimeMSecPtr,
                              bool firstPacket)
{
    QTSS_RTPSessionState *theState = NULL;
    UInt32 theLen = 0;
    ...
    for (UInt32 z = 0;
        ...
        if (this->PacketMatchesStream(inStreamCookie, theStreamPtr)) {
            ...
            if (!this->PacketReadyToSend(theStreamPtr, &currentTime, inFlags, packetIDPtr,
            ...
            if (writeErr == QTSS_WouldBlock) {
                ...
                *timeToSendThisPacketAgain = thePacket.suggestedWakeupTime;
                ...
                { // increment packet counts
                    ...
                    (void) QTSS_GetValuePtr(*theStreamPtr, sStreamPacketCountAttr, 0, (void **) &packetCountPtr,
                                            &theLen);
                    if (theLen > 0) {
                        *packetCountPtr += 1;
