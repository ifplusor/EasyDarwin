...
public:
	...
	void Reset()
	{ // make packet ready to reuse fQueueElem is always in use
...
public:
	...
	{
		OSMutexLocker locker(this->GetDemuxer()->GetMutex());
		if (inSession == fBroadcasterClientSession) fBroadcasterClientSession = NULL;
	}
...
public:
	...
	OSQueueElem *
	SendPacketsToOutput(ReflectorOutput *theOutput, OSQueueElem *currentPacket, SInt64 currentTime, SInt64 bucketDelay,
						bool firstPacket);
...
public:
	...
	// by channel numbers
	void SetRTPChannelNum(SInt16 inChannel)
	{ fRTPChannel = inChannel; }

	void SetRTCPChannelNum(SInt16 inChannel)
	{ fRTCPChannel = inChannel; }
	...
	SourceInfo::StreamInfo *GetStreamInfo()
	{ return &fStreamInfo; }

	OSMutex *GetMutex()
	{ return &fBucketMutex; }

	void *GetStreamCookie()
	{ return this; }

	SInt16 GetRTPChannel()
	{ return fRTPChannel; }

	SInt16 GetRTCPChannel()
	{ return fRTCPChannel; }

	UDPSocketPair *GetSocketPair()
	{ return fSockets; }

	ReflectorSender *GetRTPSender()
	{ return &fRTPSender; }

	ReflectorSender *GetRTCPSender()
	{ return &fRTCPSender; }

	void SetHasFirstRTCP(bool hasPacket)
	{ fHasFirstRTCPPacket = hasPacket; }

	bool HasFirstRTCP()
	{ return fHasFirstRTCPPacket; }

	void SetFirst_RTCP_RTP_Time(UInt32 time)
	{ fFirst_RTCP_RTP_Time = time; }

	UInt32 GetFirst_RTCP_RTP_Time()
	{ return fFirst_RTCP_RTP_Time; }

	void SetFirst_RTCP_Arrival_Time(SInt64 time)
	{ fFirst_RTCP_Arrival_Time = time; }

	SInt64 GetFirst_RTCP_Arrival_Time()
	{ return fFirst_RTCP_Arrival_Time; }


	void SetHasFirstRTP(bool hasPacket)
	{ fHasFirstRTPPacket = hasPacket; }

	bool HasFirstRTP()
	{ return fHasFirstRTPPacket; }

	UInt32 GetBufferDelay()
	{ return ReflectorStream::sOverBufferInMsec; }

	UInt32 GetTimeScale()
	{ return fStreamInfo.fTimeScale; }

	UInt64 fPacketCount;

	void SetEnableBuffer(bool enableBuffer)
	{ fEnableBuffer = enableBuffer; }

	bool BufferEnabled()
	{ return fEnableBuffer; }
	...
	ReflectorSession *GetMyReflectorSession()
	{ return fMyReflectorSession; }
