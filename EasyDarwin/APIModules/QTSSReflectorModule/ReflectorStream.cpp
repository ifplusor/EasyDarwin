...
void ReflectorStream::Register()
{
    ...
    (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType, sCantBindReflectorSocket, NULL,
    ...
    (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType, sCantJoinMulticastGroup, NULL,
                                   qtssAttrDataTypeCharArray);
...
QTSS_Error
ReflectorStream::BindSockets(QTSS_StandardRTSP_Params *inParams, UInt32 inReflectorSessionFlags, bool filterState,
                             UInt32 timeout)
{
    ...
    if (qtssRTPTransportTypeTCP == fTransportType) {
        ...
        if (isMulticastDest) {
            ...
            fSockets = sSocketPool.GetUDPSocketPair(fStreamInfo.fDestIPAddr, fStreamInfo.fPort, fStreamInfo.fSrcIPAddr,
                                                    0);
        }
                ...
                while (!qIter.IsDone()) {
                    ...
                    if (!dodBookmarkPacket) {
                        ...
                        err = theOutput->WritePacket(&thePacket->fPacketPtr, fStream, fWriteFlag, packetLateness,
                                                     &timeToSendPacket, NULL, NULL, false);
...
OSQueueElem *
ReflectorSender::SendPacketsToOutput(ReflectorOutput *theOutput, OSQueueElem *currentPacket, SInt64 currentTime,
                                     SInt64 bucketDelay, bool firstPacket)
{
    ...
    while (!qIter.IsDone()) {
        ...
        if (err == QTSS_WouldBlock) { // call us again in # ms to retry on an EAGAIN
            ...
            if ((timeToSendPacket > 0) &&
                ((fNextTimeToRun + currentTime) > timeToSendPacket)) // blocked but we are scheduled to wake up later
...
bool ReflectorSocket::ProcessPacket(const SInt64 &inMilliseconds, ReflectorPacket *thePacket, UInt32 theRemoteAddr,
                                    UInt16 theRemotePort)
{
    ...
    if (thePacket != NULL) {
        do {
            ...
            if (!(thePacket->IsRTCP()) && (streamInfo->fPayloadType == qtssVideoPayloadType) &&
            ...
            if (!(thePacket->IsRTCP()) && (streamInfo->fPayloadType == qtssAudioPayloadType) &&
            ...
            if (ReflectorStream::sUsePacketReceiveTime && thePacket->fPacketPtr.Len > 12) {
                ...
                if (0 == ::strncmp(theTag, "aktt", 4)) {
                    ...
                    UInt32 theSSRC = thePacket->GetSSRC(
                            theRemotePort & 1); // use to check if broadcast has restarted so we can reset
